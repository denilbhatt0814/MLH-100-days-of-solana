...
async function connectWallet(wallet) {
  ...
  if (!connectFeature) {
    errorDiv.textContent = "Selected wallet does not support connecting.";
    return;
  }
  ...
  try {
    ...
    if (accounts.length === 0) {
      errorDiv.textContent =
        "No accounts found in the wallet. Did you reject the request?";
      return;
    }
    ...
    const { values: balanceInLamports } = await rpc.getBalance(address).send();
    const balanceInSOL = (Number(balanceInLamports ?? 0) / 1e9).toFixed(9);

    walletListDiv.style.display = "none";
    statusDiv.textContent = "";
    connectedDiv.style.display = "block";
    connectedDiv.innerHTML = `
    <h3> Connected Wallet: ${wallet.name} </h3>
...
async function disconnectWallet(wallet) {
  ...
  statusDiv.textContent = "Wallet disconnected. Choose a wallet to reconnect.";
}
